...
class HotWord(FeatureFinder):
    def getFeatureTitle(self):
        return "Dangerous Body"
    
    def getFeature(self, message):
        import re
        test=" "
        test1=" "
        super(HotWord, self).getFeature(message)
        a= test.join(utils.getall(message))
        cleanPayload = re.sub(r'\s+', '', a)
        linkregex = re.compile(HOTWORD, re.IGNORECASE)
        links = linkregex.findall(cleanPayload)

        a1=test1.join(utils.getword_subject(message))
        cleanPayload1 = re.sub(r'\s+', '', a1)
        linkregex1 = re.compile(LISTSUBJECT, re.IGNORECASE)
        links1 = linkregex1.findall(cleanPayload1)
        return len(links)-1-len(links1)

class File(FeatureFinder):
    def getFeatureTitle(self):
        return "File"
    ...
    def getFeature(self, message):
        import re
        test=" "
        super(File, self).getFeature(message)
        a= test.join(utils.getword_path(message))
        cleanPayload = re.sub(r'\s+', '', a)
        linkregex = re.compile(PATH, re.IGNORECASE)
        links = linkregex.findall(cleanPayload)
        return len(links)
    
class SubjectDangerous(FeatureFinder):
    def getFeatureTitle(self):
        return "Dangerous Subject"
    
    def getFeature(self, message):
        import re
        test=" "
        super(SubjectDangerous, self).getFeature(message)
        a=test.join(utils.getword_subject(message))
        cleanPayload = re.sub(r'\s+', '', a)
        linkregex = re.compile(LISTSUBJECT, re.IGNORECASE)
        links = linkregex.findall(cleanPayload)
        return len(links)
...
def processFile(filepath, phishy=True, limit=500):
    ...
    finders = [HTMLFormFinder(), AttachmentFinder(), FlashFinder(),
               IFrameFinder(), HTMLContentFinder(), URLsFinder(), File(),
               ExternalResourcesFinder(), JavascriptFinder(), HotWord(), SubjectDangerous(),
...
def mboxtests():
    processFile("resources/spear.mbox", limit=500000)
    processFile("resources/email-enron.mbox", limit=500000, phishy=False)
