...
def getword_string(string):
    result = []

    links = re.findall(HOTWORD, string.as_string().lower())
...
def getword_path(string):
    result = []

    links = re.findall(PATH, string.as_string().lower())
...
def getword_subject(string):
    result=[]
    
    links=re.findall(LISTSUBJECT, string['subject'].lower())
    for link in links:
        if ds(link, LISTSUBJECT):
            result.append(link)
...
def getall(string):
    ...
    cleantext=re.sub("<.*?>|(?s)<style>(.*?)<\/style>", "",string.as_string().lower())
    links=re.findall(HOTWORD, cleantext.lower())
    for link in links:
        if ds(link, HOTWORD):
            result.append(link)
    return result
...
def ds(link, hot):
    return re.compile(hot, re.IGNORECASE).search(link) is not None
